...
public class OpticalBookingAdvancePage extends OpticalBookingPage {
 ...
 // ===== BOOKING / DELIVERY / ADVANCE =====
  ...
  public void payByCash(String amount) {
    waitUntilInputValueIs(advanceAmountInput, amount);

    selectPaymentMode(cashCheckbox);

    wait.until(driver -> {
        try {
            WebElement input = driver.findElement(cashAmountInput);
    ...
    clearAndEnterAmount(cashAmountInput, amount);
    waitUntilInputValueIs(cashAmountInput, amount);
